...
func TestExecutePlanRequireApprovalAutoExecutesMutatingTool(t *testing.T) {
	tempDir := t.TempDir()
	...
	if string(content) != "allowed" {
		t.Fatalf("written content = %q, want allowed", string(content))
	}
	if got, want := report.Summary.SucceededCount, 1; got != want {
		t.Fatalf("succeeded_count = %d, want %d", got, want)
	}
	if len(report.ApprovalRequests) != 0 {
		t.Fatalf("approval request count = %d, want 0", len(report.ApprovalRequests))
	}
...
func TestExecutePlanRequestPermissionsAutoGrantsUnderAutoMode(t *testing.T) {
	...
	if report.Results[0].Metadata.Permission != ToolPermissionAllowed {
		t.Fatalf("permission = %s, want %s", report.Results[0].Metadata.Permission, ToolPermissionAllowed)
	}
