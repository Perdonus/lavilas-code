...
func Catalog() *CommandCatalog {
	defaultCatalogOnce.Do(func() {
		defaultCatalog = NewCommandCatalog([]CatalogEntry{
			...
			{
				PresentationOrder:  85,
				Name:               "setlang",
				Category:           "config",
				Tags:               []string{"language", "locale", "ui"},
				EnglishAliases:     []string{"set-lang", "language"},
				EnglishDescription: "Switch the interface language",
				RussianName:        "язык",
				RussianAliases:     []string{"set-lang", "setlang"},
				RussianDescription: "Переключить язык интерфейса"},
			{
				PresentationOrder:  88,
				Name:               "permissions",
				Category:           "config",
				Tags:               []string{"permissions", "approvals", "tools"},
				EnglishAliases:     []string{"approvals"},
				EnglishDescription: "Inspect the current tool permission policy",
				RussianName:        "разрешения",
				RussianAliases:     []string{"подтверждения"},
				RussianDescription: "Посмотреть текущую политику доступа к инструментам"},
			{
