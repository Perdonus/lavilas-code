...
func (c Config) withDefaults() Config {
	...
	if strings.TrimSpace(result.Name) == "" {
		result.Name = "responses-api"
	}
	if strings.TrimSpace(result.BaseURL) == "" {
		result.BaseURL = DefaultBaseURL
	}
	if strings.TrimSpace(result.ResponsesPath) == "" {
		result.ResponsesPath = DefaultResponsesPath
	}
	if result.Timeout <= 0 {
...
func (c Config) BuildHeaders(accept string) http.Header {
	...
	headers := make(http.Header, len(normalized.Headers)+4)
