...
pub(crate) use selection_popup_common::legacy_selection_row_style;
pub(crate) use selection_popup_common::selection_highlight_style;
pub(crate) use selection_popup_common::set_selection_highlight_fill;
pub(crate) use selection_popup_common::set_selection_highlight_preset;
pub(crate) use selection_popup_common::set_selection_highlight_text_formats;
...
impl BottomPane {
    ...
    pub fn handle_mouse_event(&mut self, mouse_event: MouseEvent) {
        if self.view_stack.is_empty() {
            let (_, needs_redraw) = self.composer.handle_mouse_event(mouse_event);
            if needs_redraw {
                self.request_redraw();
            }
            return;
        ...
        let view_complete = {
            let last_index = self.view_stack.len() - 1;
            let view = &mut self.view_stack[last_index];
            view.handle_mouse_event(mouse_event);
            view.is_complete()
        };
    ...
    /// Update the status indicator header (defaults to "Выполняю") and details below it.
    ///
    ...
    pub fn push_user_input_request(&mut self, request: RequestUserInputEvent) {
        ...
        self.set_composer_input_enabled(
            /*enabled*/ false,
            Some("Ответьте на вопросы, чтобы продолжить.".to_string()),
        );
        self.push_view(Box::new(modal));
    }
    ...
    pub(crate) fn push_mcp_server_elicitation_request(
            ...
            && let Some(install_url) = tool_suggestion.install_url.clone()
        {
            ...
            let view = AppLinkView::new(
                AppLinkViewParams {
                    ...
                    instructions: match suggestion_type {
                        AppLinkSuggestionType::Install => {
                            "Установите это приложение в браузере и вернитесь сюда.".to_string()
                        }
                        AppLinkSuggestionType::Enable => {
                            "Включите это приложение, чтобы использовать его в текущем запросе."
                                .to_string()
                        }
                    },
                    url: install_url,
            ...
            self.set_composer_input_enabled(
                /*enabled*/ false,
                Some("Ответьте на предложение инструмента, чтобы продолжить.".to_string()),
            );
            self.push_view(Box::new(view));
            return;
        }
        ...
        self.set_composer_input_enabled(
            /*enabled*/ false,
            Some("Ответьте на запрос MCP-сервера, чтобы продолжить.".to_string()),
        );
        self.push_view(Box::new(modal));
    }
...
mod tests {
    ...
    fn ctrl_c_on_modal_consumes_without_showing_quit_hint() {
        ...
        let mut pane = BottomPane::new(BottomPaneParams {
            ...
            placeholder_text: "Спросите Lavilas Codex о чём угодно".to_string(),
    ...
    fn composer_shown_after_denied_while_task_running() {
        ...
        assert!(
            row0.contains("Выполняю"),
            "ожидался заголовок «Выполняю» после отказа в строке 0: {row0:?}"
        );
    ...
    fn status_indicator_visible_during_command_execution() {
        ...
        assert!(bufs.contains("• Выполняю"), "ожидался заголовок «Выполняю»");
    }
